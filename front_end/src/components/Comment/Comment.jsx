...
function Comment({ comment, currentUser, onwerPostId, setComments, setCountComments }) {
  ...
  // Keep local edit value in sync if comment gets updated via realtime
  useEffect(() => {
    setEditComment(comment?.content || "");
  }, [comment?.content]);

  // FIX crash: focus only after Textarea is mounted
  useEffect(() => {
    ...
    try {
      ...
      setComments((prevComments) =>
        prevComments.map((prevComment) => {
          ...
          // Defensive merge: avoid UI crash if backend ever returns missing relations
          return {
            ...prevComment,
            ...data,
            User: data?.User || prevComment?.User
          };
        })
      );
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      // Backend requires userId; postId is optional but allows post owner to delete.
      await makeRequest.delete(
        `/comments/${comment.id}?postId=${comment.postId}&userId=${currentUser.id}`
      );

      // Update local list immediately; socket handler will ignore if already removed
      setComments((prevComments) =>
        prevComments.filter((prevComment) => prevComment.id !== comment.id)
      );
      setCountComments((prevCountComment) =>
        typeof prevCountComment === 'number' ? Math.max(prevCountComment - 1, 0) : prevCountComment
      );
    } catch (error) {
      toast({
